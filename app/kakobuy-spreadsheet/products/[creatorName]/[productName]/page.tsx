...
export async function generateMetadata({
  ...
  const seoDescription = `Find ${product.name} by ${creatorName} on KakoBuy Spreadsheet. Best price: ¥${product.price} CNY ($${(product.price * 0.14).toFixed(2)} USD). ${
    product.description?.slice(0, 160) || 'Chinese product listing with verified details.'
  }`;
  const imageUrl =
          ...
          <div className="flex justify-center">
            <div className="rounded-lg overflow-hidden h-96 w-full relative">
              <Image
                ...
                alt={
                  product.name
                    ? `${product.name} - KakoBuy Spreadsheet`
                    : 'KakoBuy Spreadsheet Product'
                }
                fill
          ...
          <div className="flex flex-col">
            ...
            <div className="mt-6">
              ...
              <p className="text-gray-700">
                {product.description ||
                  'Explore this product on KakoBuy Spreadsheet for detailed information and pricing.'}
              </p>
            </div>
              ...
              {product.images.slice(1).map((img, index) => {
                ...
                return (
                  ...
                  >
                    <Image
                      src={src}
                      alt={`${product.name || 'KakoBuy Product'} - Image ${
                        index + 2
                      }`}
                      fill
